...
-- MAGIC 
-- MAGIC In order to grant access to a specific group of users, we need to create that group and then set permission levels. In order to do this, you would need admin acess to your workspace. You can head over to [Manage Principals in Unity Catalog]($./DE 6.99 - OPTIONAL Administration/DE 6.99.1 - Manage principals in Unity Catalog) for a step by step walkthrough. 
...
-- MAGIC 
-- MAGIC Now we'll try the function as an analayst in the Databricks SQL session. Paste the fully qualified query statement generated below into a new query to run this function in the Databricks SQL session. Since all appropriate grants are in place for analysts to access the function, the output should resemble what we just saw above.
