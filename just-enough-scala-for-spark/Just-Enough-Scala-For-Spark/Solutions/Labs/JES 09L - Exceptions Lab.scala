...
class InvalidBodyParametersException(message: String) extends Exception(message) {
  // No code here 
}
...
def getBMIStats(weight: Double, height: Double, bmiCutoff: Double = 25.0): String = {
  ...
  // Add try-catch
  try {
    // Add code to check if height or weights are more than zero and throw an exception if not
    
    if (height <= 0.0 | weight <= 0.0) {
      // Throw an exception with invalid age message
      throw new InvalidBodyParametersException("Height or weight can not be zero or negative values.")
    }
  ...
  // Add the catch statements  
    ...
    // Deal with invalid data exception
    case e: InvalidBodyParametersException => return "INVALID INPUTS"
    // Deal with invalid height or weight
    case e: Exception => return "INVALID INPUTS"
  }
}
...
var firstAdvisory = getBMIStats(140.0, 70.0)
var secondAdvisory = getBMIStats(140.0, 0.0)
var thirdAdvisory = getBMIStats(60.0, -1.0)
