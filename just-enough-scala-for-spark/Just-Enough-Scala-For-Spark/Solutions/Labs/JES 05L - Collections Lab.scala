...
var vectorOfWords = Vector("the", "dog", "visited", "the", "firehouse")
...
var uniqueWordsVector = vectorOfWords.distinct
...
val sortedWordsVector = uniqueWordsVector.sorted
...
val reversedWordsVector = sortedWordsVector.reverse

for (word <- reversedWordsVector) {
  println(word)
}
...
def weather(temperature: Double, humidity: Double): (String, String) = {
  
  var heat = ""
  var moisture = ""
  
  // Determining heat based on temperature
  if (temperature > 80) {
    heat = "Hot"
  ...
  // Deterimining moisutire variable based on temperature and humidity
  if (humidity > 40 & temperature > 50) {
    moisture = "Humid"
  } else if (humidity > 40 & temperature <= 50) {
    moisture = "Damp"
  } else {
    moisture = "Pleasant"
  }
  
  // Returning a tuple with both heat and moisture
  (heat, moisture)
  // Can also be written as below with the return keyword
...
val (heatOne, moistureOne) = weather(100, 60)
...
val allBaskets = basket1 ++ basket2 ++ basket3

// Find the unique set of items bought across all the baskets
val uniqueSetOfItems = allBaskets.toSet
...
val commonSetOfItems = basket1.toSet.intersect(basket3.toSet)
...
var customerInfo = scala.collection.mutable.Map("sally@taylor.com" -> ("Sally Taylor", 32) )
...
customerInfo("jiminy@cricket.com") = ("Jiminy Cricket", 45)
